...
**[11. Схема проекта](#11-список-серверов)**
...
Ссылка: [Схема таблиц в dbdiagram.io](https://dbdiagram.io/d/HighLoad_Logic_DB-68e7bbecd2b621e42211ffad)
...
Ссылка: [Физическая схема таблиц в dbdiagram.io](https://dbdiagram.io/d/HighLoad_Physical_DB-68f3df852e68d21b41201a1d)
...
|-------------------|------------------------|
| **L7 Балансировщики (NGINX)** | - **Распределение**: Active-Active кластер с N+1 репликами по трём availability-зонам с Anycast BGP балансировкой трафика [[27]](#источники)<br>- **Отказоустойчивость**: Автоматическое переключение трафика при отказе зоны<br>- **Защита**: Rate limiting для предотвращения перегрузки<br>- **Мониторинг**: Health checks с автоматическим исключением и рестартом неработающих нод |
| **Сервера приложений (микросервисы)** | - **Оркестрация**: Kubernetes ReplicaSet с N+1 по трём availability-зонам репликами на сервис<br>- **Управление нагрузкой**: Автомасштабирование по метрикам CPU и памяти<br>- **Безопасное завершение**: Graceful Shutdown для сохранения целостности данных<br>- **Деградация**: Graceful Degradation при частичных отказах<br>- **Самовосстановление**: Health checks с автоматическим рестартом контейнеров |
| **Базы данных PostgreSQL** | - **Оркестрация**: Patroni для автоматического управления кластером<br>- **Доступность**: реверс-проксёй Odyssey обслуживать много клиентов, используя небольшой пул соединений к БД<br>- **Репликация**: Master + 2 реплики<br>- **Резервное копирование**: WAL + полные снепшоты |
| **Кэш Redis (сессии)** | - **Оркестрация**: Redis Sentinel для мониторинга и автоматического failover<br>- **Кластеризация**: Redis Cluster с архитектурой master-slave<br>- **Синхронизация**: Репликация в реальном времени между узлами<br>- **Персистентность**: Комбинирование RDB и AOF для сохранения данных |
| **Дата-центры** | - **Инфраструктура**: Tier-4 архитектура<br>- **Сети**: Избыточная схема сети<br>- **Энергоснабжение**: Схема подачи питания<br>- **Охлаждение**: Избыточная система охлаждения [(приложение D)](#приложение-d)<br>- **География**: Стратегическое физическое положение для минимизации рисков |
...
![Схема проекта](/materials/highload_scheme_project.png)

## 11. Список серверов

### Расчёт для сервисов

Пусть на 1 CPU (ядро) для Go сервиса приходится **1000 RPS** [[28]](#источники).

| Ресурс | RPS | Пиковый RPS | Кол. ядер = &lceil; Пиковый RPS / RPS ядра &rceil; | Кол. 4-ядерных машин с N+1 резервом | CPU на контейнер |
|--------|-----|-------------|---------------------------------------------------|--------------------------------------|------------------|
| Микросервис Transaction | 2 607 | 7 818 | 8 | 2 + 1 резерв = **3** | 4 |
| Микросервис Wallet | 1 598 | 4 794 | 5 | 2 + 1 резерв = **3** | 4 |
| Микросервис API | 16 | 48 | 1 | 1 + 1 резерв = **2** | 4 |
| NGINX L7-балансировщики для доменов | [см. расчеты](#схема-балансировки-нагрузки) | [см. расчеты](#схема-балансировки-нагрузки) | [см. расчеты](#схема-балансировки-нагрузки) | 3 домена × 3 машины = **9** | - |

### Расчёт для хранилища PostgreSQL

**Исходные данные** [[см. раздел 5]](#5-логическая-схема-бд):
...
- Если размер данных на диске >1 ТБ, то рекомендуемый размер RAM >256 ГБ [[30]](#источники)
- Используем 1-4 CPU на каждую активно используемую базу [[30]](#источники)
- Пусть 1 ядро PostgreSQL обрабатывает **200 операций/сек**

**Расчёт:**
- Требуемое количество ядер на чтение: &lceil;9 425 / 200&rceil; = **48 ядер** → **2** 24-ядерные машины для реплик
- Требуемое количество ядер на запись: &lceil;5 222 / 200&rceil; = **27 ядер** → **1** 32-ядерная машина для мастера

### Расчёт для хранилища Redis

**Исходные данные** [[см. раздел 5]](#5-логическая-схема-бд):
- Объём данных на диске: **113 МБ**
- Нагрузка на чтение: **16 QPS**
- Нагрузка на запись: **1 QPS**
- Суммарная нагрузка: **17 QPS**

**Допущения:**
- Пусть RPS Redis равен **35 600** единицам [[29]](#источники)

**Вывод:**
- Требуется **1 ядро**, RAM **>1 ГБ**
...
27. [Кластеризация для обеспечения отказоустойчивости | Microsoft Learn](https://learn.microsoft.com/ru-ru/windows-server/failover-clustering/failover-clustering-overview)
28. [Сколько нужно ядер cpu, чтобы выдержать 30k+ rps? / Хабр](https://habr.com/ru/articles/773680/)
29. [Нагрузочный поединок: Tarantool 2.10 vs Redis 7.0.5 / Хабр](https://habr.com/ru/companies/kts/articles/730518/#7)
30. [Все, что нужно PostgreSQL: быстрые диски, дорогой процессор и терабайты RAM / Хабр](https://habr.com/ru/companies/selectel/articles/912996/#2)
...
![Схема охлаждения](/materials/shema_window.png)
