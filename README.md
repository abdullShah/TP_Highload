...
Для Go сервисов используем оценку: **1000 RPS на 1 ядро CPU** [[28]](#источники).
...
Для NGINX используем оценку: **428 CPS (connections per second) на 1 ядро CPU для HTTPS** [[22]](#источники).

| Компонент        | Пиковый RPS | Требуется ядер CPU                                                | RAM на экземпляр | Назначение                                            |
| ---------------- | ----------- | ----------------------------------------------------------------- | ---------------- | ----------------------------------------------------- |
| NGINX L7 (всего) | 12 662      | ⌈12662 / 428⌉ = **30** ядер<br>С учетом 60% загрузки: **50 ядер** | 4 ГБ             | SSL Termination, маршрутизация по доменам             |
...
**Исходные данные** [[см. раздел 5 и 6]](#5-логическая-схема-бд):
...
| Компонент                    | Объём данных | Количество шардов | Серверов (основных) | Серверов (реплик) | CPU на сервер | RAM на сервер | Диск на сервер | Назначение                                                      |
| ---------------------------- | ------------ | ----------------- | ------------------- | ----------------- | ------------ | ------------- | -------------- | --------------------------------------------------------------- |
| PostgreSQL Shards (credit)   | 587 ТБ       | 587                | 74                  | 74                | 32 ядра      | 128 ГБ        | 10 ТБ          | Хранение исторических транзакций зачислений, низкая нагрузка     |
| PostgreSQL Shards (debit)    | 587 ТБ       | 587                | 74                  | 74                | 32 ядра      | 128 ГБ        | 10 ТБ          | Хранение исторических транзакций списаний, низкая нагрузка      |
| PostgreSQL Receipts          | 7.6 ТБ       | 1                  | 1                   | 1                 | 32 ядра      | 128 ГБ        | 10 ТБ          | Хранение исторических чеков, низкая нагрузка                     |
...
-   RPS Redis равен **35 600** единицам [[29]](#источники)
...
| Аренда VM  | 3        | 42         | 128 ГБ      | $10 800          | $180       |
| **ВСЕГО**  | **322**  | **9 578**  | **38.8 ТБ** | **$4 797 800**  | **$79 879** |
