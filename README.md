...
- горизонтальный: поскольку максимальный размер таблицы	32 ТБ [[25]](#источники), то таблицу разнесем на 587 ТБ / 32 ТБ = 18.3 ≈ **19 частей** по ключу ```to_account```
...
- горизонтальное: поскольку максимальный размер таблицы	32 ТБ [[25]](#источники), то таблицу разнесем на 587 ТБ / 32 ТБ = 18.3 ≈ **19 частей** по ключу ```from_account```
...
**Распределенная блокировка** (Distributed Lock) - это механизм, который позволяет только одному узлу или процессу в распределенной системе получать блокировку на общий ресурс [[27]](#источники) | Может быть такое, что один процесс обновил значение, но другой процесс все еще работает со старым значением.<br><br>Например, когда несколько транзакций могут пытать обновить баланс одного и того же счета.<br><br>Или например, два экземпляра сервиса ```transactions``` одновременно обрабатывают один и тот же вклад в ```deposits``` для начисления процентов | Предотвращение *гонки состояний*

#### Сводная таблица алгоритмов

| Алгоритм | Область применения | Мотивация |
|-------------------|-------------------|-----------|
| **Автоматический выключатель (Circuit Breaker)** | Микросервисы `wallet`, `transaction`, `api` при взаимодействии друг с другом и с БД | Если один из микросервисов или реплика БД начинает отвечать с ошибками или слишком медленно, то разрывает связь после серии неудач, перенаправляя трафик, давая проблемному узлу время на восстановление |
| **BGP Anycast** | Балансировка L7-уровня внутри ДЦ | Автоматическое переключение при сбоях availability-зон, равномерное распределение нагрузки внутри каждого домена |
| **Write-Ahead Log (WAL)** | PostgreSQL для таблиц `users`, `accounts`, `transactions` | Обеспечение надежности и согласованности данных при сбоях |
| **Master-Slave Репликация** | Слой БД: Master для записи, 2 реплики для чтения | Масштабирование чтений (1230 RPS баланса) |
...
27. [How Distributed Lock works | ft Redis | System Design - YouTube](https://www.youtube.com/watch?v=qY4MfWv01pI)
